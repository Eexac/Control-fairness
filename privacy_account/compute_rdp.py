...
def compute_rdp_accumulation(q, noise_multiplier, privacy_cost_before, orders):
    """Computes RDP of the Sampled Gaussian Mechanism. accumulate the privacy before
  Args:
    q: The sampling rate.
    noise_multiplier: The ratio of the standard deviation of the Gaussian noise    STD标准差，敏感度应该包含在这里面了
      to the l2-sensitivity of the function to which it is added.
  ...
  """
    if np.isscalar(orders):
        ...
        rdp = np.array(
            [_compute_rdp(q, noise_multiplier, order)+privacy for (order,privacy) in zip(orders,privacy_cost_before)])
    return rdp
...
if __name__ == '__main__':
    #ORDERS = list(range(2, 64)) + [128]
    # [1 + x / 10.0 for x in range(1, 100)] +  this is the first code
    # q = 3000 / 60000
   ...
   # ORDERS = list(range(2, 64)) + [128]
    ...
    #rdp = compute_rdp(0.32, 5.67, 140, ORDERS)
   # dp, order = compute_eps(ORDERS, rdp, 1e-5)
   # print("rdp:", rdp)
   # print("dp:", dp)
   # print("order:", order)
    orders = list(range(11, 64)) + [128]
    rdp_list = [0] * len(orders)
    for i  in range(900):
       epsilon, order, rdp_list = compute_rdp_accumulation(0.068, 5.67, rdp_list, orders, 0.00001)
       print(f'epsilon: {epsilon},step: {i}')
