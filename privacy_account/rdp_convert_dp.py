...
def compute_delta(orders, rdp, epsilon):
   ...
   """
    ...
    if epsilon <= 0:
        raise ValueError("Privacy failure probability bound epsilon must be >0.")
    ...
    for (a, r) in zip(orders_vec, rdp_vec):
        ...
        if r >= epsilon:
            ...
            delta = math.exp(-(epsilon - r) * (a - 1))
        else:
            delta = np.inf
        delta_vec.append(delta)

    idx_opt = np.argmin(delta_vec)
    return max(0, delta_vec[idx_opt]), orders_vec[idx_opt]
